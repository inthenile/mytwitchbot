...
async def check_game_status():
    ...
    # get the first person to have been added to the list, and declare them the winner.
        if len(list_of_users) >= 1:
            winner = list_of_users[0]
            await my_bot.bot.connected_channels[0].send(f"Winner is {winner}")
        ...
        # if no one repeated the bot and the timer ran out
            ...
            await my_bot.bot.connected_channels[0].send(f"Time's up. Next time, repeat the {my_bot.bot.game_word} to win!")
            break
